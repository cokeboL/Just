...
public class Main {
    ...
    private static void readData(Selector selector, SelectionKey selectionKey) throws IOException {
        ...
        System.out.println("上一次读取的数据："+oldBuffer+getLineEnd()+toDisplayChar(oldBuffer));
        ...
        System.out.println("这次读取的数据："+newBuffer+getLineEnd()+toDisplayChar(newBuffer));
        
        
        newBuffer.flip();
        ByteBuffer lineRemain = getLine(newBuffer);
        
        System.out.println("解析的行数据剩余部分："+lineRemain+getLineEnd()+toDisplayChar(lineRemain));
        
        if (null != lineRemain) { // 获取到行结束符
            ByteBuffer completeLine = merge(oldBuffer, lineRemain);
            
            System.out.println("准备输出的数据："+completeLine+getLineEnd()+toDisplayChar(completeLine));
            ...
            System.out.println("判断是否退出的行数据："+lineStr);
            ...
            System.out.println("暂存到SelectionKey的数据："+temp+getLineEnd()+toDisplayChar(temp));
    ...
    private static SocketChannel acceptNew(Selector selector,
        ...
        if (null != socketChannel) {
            System.out.println("收到一个新的连接，客户端IP："+socketChannel.socket().getInetAddress().getHostAddress()+"，客户端Port："+socketChannel.socket().getPort());
    ...
    private static void startServer(int port, int selectTimeout) {
        ...
        try {
            ...
            System.out.println("NIO echo网络服务启动完毕，监听端口：" +port);
        	...
        	System.out.println("处理网络连接出错"+e.toString());
        }
    }
